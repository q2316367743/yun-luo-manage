    ...
    <el-card class="server-info">
        ...
        <el-tabs tab-position="top" style="margin-top: 52px" v-model="index">
            <el-tab-pane label="基础信息" name="base">
                <el-form label-position="right" label-width="70px" style="margin-top: 22px">
                    <el-form-item label="名称：">
                        <el-input v-model="server.name" v-if="is_update" style="width: 400px"></el-input>
                        <div v-else>{{ server.name }}</div>
                    </el-form-item>
                    <el-form-item label="类型：">
                        <el-input v-model="server.type" v-if="is_update" style="width: 400px"></el-input>
                        <div v-else>{{ server.type }}</div>
                    </el-form-item>
                    <el-form-item label="版本：">
                        <el-input v-model="server.version" style="width: 400px" v-if="is_update"></el-input>
                        <div v-else>{{ server.version }}</div>
                    </el-form-item>
                    <el-form-item>
                        <el-button @click="is_update = !is_update">{{ is_update ? "取消" : "修改" }}
                        </el-button>
                        <el-button @click="reset" v-if="is_update">重置</el-button>
                        <el-button type="primary" @click="update" v-if="is_update">修改</el-button>
                    </el-form-item>
                </el-form>
            </el-tab-pane>
            <el-tab-pane label="操作命令" name="command">
                <el-table :data="commands" style="width: 100%; margin-top: 22px">
                    ...
                    <el-table-column label="操作">
                        <template slot-scope="scope">
                            ...
                            <el-button type="text" @click="command_remove(scope.row.id)">删除</el-button>
                        </template>
                    </el-table-column>
                </el-table>
            </el-tab-pane>
                ...
                <el-table :data="configs" style="width: 100%; margin-top: 22px">
                    ...
                    <el-table-column label="操作" width="180">
                        <template slot-scope="scope">
                            ...
                            <el-button type="text" @click="config_remove(scope.row.id)">删除</el-button>
                        </template>
                    </el-table-column>
                </el-table>
            </el-tab-pane>
        </el-tabs>
        <el-dialog :title="command_is_update ? '修改' : '新增' + '命令'" :visible.sync="command_dialog" v-if="command_dialog" destroy-on-close append-to-body :modal-append-to-body="false" :close-on-click-modal="false" width="50%">
        ...
        <el-dialog :title="config_is_update ? '修改' : '新增' + '配置文件'" :visible.sync="config_dialog" v-if="config_dialog" destroy-on-close append-to-body :modal-append-to-body="false" :close-on-click-modal="false" width="50%" top="10vh">
            ...
            <span slot="footer" class="dialog-footer">
                ...
                >{{ config_is_update ? '修 改' : '新 增' }}</el-button
                >
            </span>
        </el-dialog>
        <div class="add_button" v-if="index === 'command' || index === 'config'">
            <el-button v-if="index === 'command'" icon="el-icon-plus" circle @click="open_command_dialog(false)" style="font-size: 34px;"></el-button>
            <el-button v-if="index === 'config'" icon="el-icon-plus" circle @click="open_config_dialog(false)" style="font-size: 34px;"></el-button>
        </div>
    </el-card>
</template>
    ...
    methods: {
        ...
        command_remove(id) {
            this.$confirm('此操作将永久删除该命令, 是否继续?', '提示', {
                ...
                server_api.command_remove(id, (res) => {
                    if (res.success) {
                        this.$message({
                            type: 'success',
                            message: '删除成功!'
        ...
        config_remove(id) {
            this.$confirm('此操作将永久删除该配置文件, 是否继续?', '提示', {
                ...
                server_api.config_remove(id, (res) => {
                    if (res.success) {
                        this.$message({
                            type: 'success',
                            message: '删除成功!'
